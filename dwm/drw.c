...
drw_text(Drw *drw, int x, int y, unsigned int w, unsigned int h, unsigned int lpad, const char *text, int invert, Bool markup)
{
	...
	while (1) {
		...
		if (utf8strlen) {
			...
			/* do NOT shorten text, do NOT add ellipsis */
			//len = MIN(utf8strlen, sizeof(buf) - 1);
			//memcpy(buf, utf8str, len);
			//buf[len] = '\0';

			/* get full width (even if it overflows) */
...
int
drw_text_status(Drw *drw, int x, int y, unsigned int w, unsigned int h, unsigned int lpad, const char *text, int invert, Bool markup)
{
	char buf[1024];
	int ty;
	unsigned int ew;
